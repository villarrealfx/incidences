...
class NDIYearExport implements FromCollection,WithHeadings,WithMapping,WithColumnFormatting,ShouldAutoSize,WithStyles,WithEvents
{
    use Exportable;
    ...
    public function __construct(int $cs_id, int $year)
    {
        ...
        $this->cs->percent = 0;
        $this->year = $year;
        $this->distribution_count = Incidence::whereSystem_id(1)->whereBetween('date', [$year.'-01-01', $year.'-12-31'])->count();
    }
    ...
    */
    ...
    {
        return [
            ['Análisis de Interrupciones (NDI) en Distribución Sucre por Centros de Servicio - Año '.$this->year],
    ...
    */
    public function collection()
    {
        ...
        foreach ($circuits as $circuit) {
            $circuit->ndi = $circuit->incidences->where('system_id','=',1)->whereBetween('date', [$this->year.'-01-01', $this->year.'-12-31'])->count();
            $circuit->programadas = $circuit->incidences->where('system_id','=',1)->whereBetween('date', [$this->year.'-01-01', $this->year.'-12-31'])->where('cause_id','=',6)->count();
            $circuit->mantenimiento = $circuit->incidences->where('system_id','=',1)->whereBetween('date', [$this->year.'-01-01', $this->year.'-12-31'])->where('cause_id','=',1)->count();
            $circuit->lluvia = $circuit->incidences->where('system_id','=',1)->whereBetween('date', [$this->year.'-01-01', $this->year.'-12-31'])->where('cause_id','=',2)->count();
            $circuit->componentes = $circuit->incidences->where('system_id','=',1)->whereBetween('date', [$this->year.'-01-01', $this->year.'-12-31'])->where('cause_id','=',3)->count();
            $circuit->vegetacion = $circuit->incidences->where('system_id','=',1)->whereBetween('date', [$this->year.'-01-01', $this->year.'-12-31'])->where('cause_id','=',4)->count();
        }
